...
function setOrderTotalAmount() {
    ...
    for (let index = 0; index < paperRow; index++) {
        try {
            let noOfSheetsEle = document.querySelector(
                "#PaperRow" + index + " td input[name='numberOfSheet']"
            );
            ...
            if (noOfSheets > 0) {
                let paperPriceEle = document.querySelector(
                    "#PaperRow" + index + " td input[name='sheetPrice']"
                );
                let paperTaxEle = document.querySelector(
                    "#PaperRow" + index + " td input[name='sheetGST']"
                );
                let paperPrice = 0;
                if (paperPriceEle !== null) {
                    ...
                    if (paperTaxEle != null) {
                        ...
                        totalOrderTax +=
                            noOfSheets * parseFloat(paperTaxEle.value);
                    }
                }
    ...
    $("#orderTotalWithoutGST").text(
        (totalOrderAmount - totalOrderTax).toFixed(2)
    );
...
function checkGoogleAuth() {
    ...
    const url =
        "http://localhost:8080/api/secured/branch/is-google-auth-allowed";
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                allProductCategories.forEach((element) => {
                    ...
                    document
                        .querySelector("#myModal form #albumType")
                        .appendChild(node);
                });
                ...
                allSizes.forEach((element) => {
                    ...
                    node.innerHTML = element;
                    document
                        .querySelector("#myModal form #albumSize")
                        .appendChild(node);
                });

                selectedProductSize = $(
                    "#myModal form #albumSize option:selected"
                ).val();
...
function changeCoverAndPaperOptions(selectedProductSize) {
    ...
    allPapers.forEach((paper) => {
        if (paper["paperSize"] === selectedProductSize) {
            const data = {
                id: paper["id"],
                name: paper["paperQuality"],
                price: paper["paperPrice"],
                gst: paper["gst"],
            };
...
function setCoverPriceInInput() {
    ...
    availableCovers.forEach((cover) => {
        ...
        if (parseInt(cover["id"]) === parseInt(id)) {
            ...
            $("#myModal form #coverTotal").val(
                parseFloat(cover["price"]) + parseFloat(cover["gst"])
            );
...
function addCoverOptions() {
    ...
    availableCovers.forEach((cover) => {
        ...
        document
            .querySelector("#myModal form #coverPageQuality")
            .appendChild(node);
...
function setPaperPriceOnChange(id) {
    // console.log(id);
    const paperId = $(
        "#myModal form #sheetType" + id + " option:selected"
    ).val();
    // console.log(paperId);

    availablePapers.forEach((paper) => {
        if (parseInt(paper["id"]) === parseInt(paperId)) {
            $("#PaperRow" + id + " td input[name='sheetPrice']").val(
                paper["price"]
            );
            $("#PaperRow" + id + " td input[name='sheetGST']").val(
                paper["gst"]
            );
            $("#PaperRow" + id + " td input[name='sheetTotal']").val(
                parseFloat(paper["price"]) + parseFloat(paper["gst"])
            );
...
function appendPaperRow() {
    ...
    <tr id="PaperRow` +
        ...
        `">
    <td> 
    <select onchange="setPaperPriceOnChange(` +
        paperRow +
        `)" class="form-control" name="paperId" id="sheetType` +
        paperRow +
        `"  style="width: 160px;">
