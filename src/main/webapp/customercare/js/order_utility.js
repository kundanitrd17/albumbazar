function fetchDeliveryAddressFromServer(orderId) {
    ...
    xhr.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            ...
            modal.querySelector('input[name="contactNo"]').value =
                address["contactNo"];
            modal.querySelector('input[name="landmark"]').value =
                address["landmark"];
            ...
            modal.querySelector('input[name="pincode"]').value =
                address["pincode"];
            modal.querySelector('input[name="district"]').value =
                address["district"];
...
function changeOrderDescription(order_id) {
    const description = document.getElementById(
        "updatableOrderDescription" + order_id
    ).value;
    ...
    const xhr = new XMLHttpRequest();
    const url = "/api/customer-care/order/" + order_id + "/description";
    ...
    xhr.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            ...
            element.querySelector("#productName").innerHTML =
                data["product_name"];
            element.querySelector("#productSize").innerHTML =
                data["product_size"];
            element.querySelector("#coverName").innerHTML =
                data["cover"]["coverName"];
            element.querySelector("#coverSize").innerHTML =
                data["cover"]["coverSize"];
...
function fetchCustomerDetails(id) {
    ...
    xhr.onreadystatechange = function () {
        if (this.readyState === 4 && this.DONE) {
            ...
            customerDetailModal.querySelector("#contact").innerHTML =
                data["contactNo"];
            customerDetailModal.querySelector("#email").innerHTML =
                data["email"];
        }
    };
